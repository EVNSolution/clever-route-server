...
Every successor publication must rebind exactly the requested number of unique orders in the same transaction; a partial ownership match aborts the archive, successor, stop, and pointer writes together. Rollback resolves archived pre-discriminator or legacy membership first, then writes a canonical schema-v1 snapshot through the same snapshot builder before publishing it, so the returned grouping and the next mutation use the same strict reader contract.
