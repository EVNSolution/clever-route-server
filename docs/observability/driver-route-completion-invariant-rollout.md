...
Every successor publication must rebind exactly the requested number of unique orders in the same transaction; a partial ownership match aborts the archive, successor, stop, and pointer writes together. Rollback first collapses each durable route-slot successor lineage to its single terminal archived snapshot, resolves every selected modern membership, and rejects ambiguous lineage or cross-route order/stop overlap before the first write. It then writes a canonical schema-v1 snapshot through the same snapshot builder. Archived snapshots without embedded membership fail closed because their shared mutable route-plan stops cannot reconstruct history; the known-legacy fallback remains read-only compatibility for a CURRENT child only.
