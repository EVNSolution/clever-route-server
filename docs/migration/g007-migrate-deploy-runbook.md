...
- Existing-schema proof is exact: the wrapper applies checked-in `migration.sql` files to the fingerprint DB from `20260520000000_initial_route_ops_baseline` through `G007_EXPECTED_APPLIED_THROUGH`, compares target to fingerprint with `prisma migrate diff --exit-code`, refuses any nonzero diff, validates existing `_prisma_migrations` rows/checksums/failures, resolves only missing history through that migration, then deploys remaining migrations. The migration chain now includes 44 migrations through `20260723013000_g010_import_row_resource_tenant_fks`.
- `apps/delivery-api/prisma/rehearsal-fingerprints/db-push-source-before-20260722233000.sql` may be applied only while building an empty fingerprint scratch DB. It must never run against the source, target, restore, recovery, or production database.
...
- baseline and every checked-in migration through `20260723013000_g010_import_row_resource_tenant_fks` deployed;
