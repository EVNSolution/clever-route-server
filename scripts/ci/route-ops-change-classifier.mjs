...
const RETENTION_RUNTIME_CONTRACT_RE = /^(?:scripts\/run-driver-event-attempt-retention\.sh|tests\/deploy\/route-ops-retention-runtime\.test\.sh|apps\/delivery-api\/(?:Dockerfile|package(?:-lock)?\.json|src\/scripts\/cleanup-(?:driver-event-attempts|shopify-webhook-events|driver-proof-media)\.ts|tests\/(?:package-scripts\.test\.ts|deploy\/driver-event-attempt-retention-schedule\.test\.sh)))$/;
