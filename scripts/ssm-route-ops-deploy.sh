...
ROUTE_OPS_DRIVER_APP_DOWNLOAD_URL_PARAMETER_NAME="${ROUTE_OPS_DRIVER_APP_DOWNLOAD_URL_PARAMETER_NAME:-/clever-route/prod/driver-app-download-url}"
export ROUTE_ENGINE_GRAPH_DEST_ROOT ROUTE_ENGINE_GRAPH_HOST_DIR ROUTE_ENGINE_GRAPH_S3_CURRENT_URI ROUTE_ENGINE_IMAGE ROUTE_OPS_WEB_STATIC_IMAGE ROUTE_OPS_WEB_STATIC_VOLUME ROUTE_OPS_DRIVER_APP_DOWNLOAD_URL_PARAMETER_NAME
...
validate_inputs() {
  ...
  if [ -n "$ROUTE_ENGINE_IMAGE" ]; then
    [[ "$ROUTE_ENGINE_IMAGE" =~ ^ghcr\.io/evnsolution/route-engine-worker:[0-9a-fA-F]{40}$ ]] || fail "ROUTE_ENGINE_IMAGE must be the approved route_engine worker GHCR image with SHA tag when set"
  fi
  ...
  [[ "$ROUTE_OPS_WEB_STATIC_VOLUME" == "clever-route-route-ops-web-static-$IMAGE_TAG" ]] || fail "frontend static volume tag must match IMAGE_TAG"
}
