...
COMPLETION_REJECT_ALARM_NAME="${DRIVER_ROUTE_COMPLETION_REJECT_CLOUDWATCH_ALARM_NAME:-clever-driver-route-completion-rejected}"
...
run aws logs put-metric-filter \
  ...
  --filter-pattern '{ $.event = "driver_route_completion_invariant" && $.wouldReject = true }' \
  --metric-transformations "metricName=DriverRouteCompletionWouldReject,metricNamespace=${METRIC_NAMESPACE},metricValue=1"

run aws logs put-metric-filter \
  --region "$AWS_REGION" \
  --log-group-name "$LOG_GROUP" \
  --filter-name driver-route-completion-rejected \
  --filter-pattern '{ $.event = "driver_route_completion_invariant" && $.decision = "REJECTED" && $.wouldReject = true }' \
  --metric-transformations "metricName=DriverRouteCompletionRejected,metricNamespace=${METRIC_NAMESPACE},metricValue=1"
...
printf 'driver event CloudWatch configuration ready: group=%s retentionDays=90 namespace=%s alarm=%s completionAlarm=%s rejectionAlarm=%s\n' \
  "$LOG_GROUP" "$METRIC_NAMESPACE" "$ALARM_NAME" "$COMPLETION_ALARM_NAME" "$COMPLETION_REJECT_ALARM_NAME"
