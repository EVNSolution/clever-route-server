...
case "$1 ${2:-}" in
  ...
  'ssm get-command-invocation')
    if [ "${MOCK_INVOCATION_STATUS:-Success}" = Success ]; then
      printf '%s\n' '{"Status":"Success","ResponseCode":0,"StandardOutputContent":"{\"mode\":\"apply\"}\n"}'
    else
...
run_mocked_wrapper() {
  ...
  CUSTOMER_EMAIL_RECONCILIATION_MANIFEST_SHA256="$(printf 'a%.0s' {1..64})" \
  CUSTOMER_EMAIL_RECONCILIATION_APPROVAL_REF='EVNSolution/clever-change-control#265:comment-123' \
  CUSTOMER_EMAIL_RECONCILIATION_APPROVER_LOGINS='release-manager' \
...
: >"$mock_log"
if run_mocked_wrapper unapproved-user >/dev/null 2>&1; then
