...
export function createDsvAdminSessionSubject(input: {
  ...
  if (!uuidPattern.test(accountId) || shopDomain === null || !Number.isSafeInteger(input.tokenVersion) || input.tokenVersion < 0) {
    throw new Error('Invalid DSV admin session subject');
  }
  return `${accountSubjectPrefix}${accountId}:${input.tokenVersion}:${shopDomain}`;
}
...
export function parseDsvAdminSessionSubject(subject: string): DsvAdminSessionSubject | null {
  if (subject.startsWith(accountSubjectPrefix)) {
    ...
    const [accountId, rawTokenVersion, rawShopDomain] = parts;
    const tokenVersion = Number(rawTokenVersion);
    const shopDomain = normalizeShopDomain(rawShopDomain);
    if (!uuidPattern.test(accountId ?? '') || !Number.isSafeInteger(tokenVersion) || tokenVersion < 0 || shopDomain === null) {
      return null;
    }
    return { accountId: accountId!, kind: 'account', shopDomain, tokenVersion };
  }
