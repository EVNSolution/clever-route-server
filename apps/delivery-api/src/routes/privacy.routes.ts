  ...
  return renderDriverPage({
    ...
    <section>
      ...
      <ul>
        ...
        <li><strong>지원 문의:</strong> 사용자가 선택적으로 제출한 문의 제목과 내용, 계정 식별자, 서버에 등록된 작성자 이름 및 접수 시각. 문의 접수와 서비스 지원을 위해 처리하며, 본인과 해당 업무 공간의 조회 권한이 있는 CLEVER 개발 관리자만 서비스 화면에서 조회할 수 있습니다. 일반 운영자나 다른 배송원에게 공개하지 않습니다.</li>
    ...
    <section>
      ...
      <p>지원 문의는 계정 유지 기간 동안 보관하며, 계정 삭제 처리가 완료될 때 문의 내용과 작성자 정보도 함께 삭제합니다. 문의에는 비밀번호, 인증 토큰 등 불필요한 민감 정보를 입력하지 마세요.</p>
...
function renderRoutesAppPrivacyPage(): string {
  return renderRoutesAppPage({
    ...
    <section>
      ...
      <p>증빙 사진과 관련 메타데이터의 기본 보관 기간은 업로드 후 <strong>기본 365일</strong>입니다. 배송 분쟁, 보안 사고, 계약 또는 법적 의무에 따른 보존 조치가 있으면 필요한 범위에서 더 오래 보관할 수 있습니다.</p>
    </section>
