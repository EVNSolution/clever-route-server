...
apply_pre_db_push_sql() {
  pre_db_push_sql_file="$1"
...
if [ -n "${PRISMA_PRE_DB_PUSH_SQL_FILE:-}" ]; then
  apply_pre_db_push_sql "$PRISMA_PRE_DB_PUSH_SQL_FILE"
else
  for migration in \
  ...
  do
    ...
    elif [ -f "prisma/migrations/$migration/migration.sql" ]; then
      apply_pre_db_push_sql "prisma/migrations/$migration/migration.sql"
    fi
  done
fi
