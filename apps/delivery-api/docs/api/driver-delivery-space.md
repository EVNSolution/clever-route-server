...
- `GET /driver/delivery-space`: `mine`, `available`, 전달 가능한 `recipients`, 낙관적 잠금용 `version` 조회
...
- `POST /driver/delivery-space/{destinationId}/transfer`: 내 배송지 전체를 선택한 배송원에게 전달
...
`mine`, `available`, `recipients`는 `Asia/Seoul` 기준 당일 그룹에서만 반환한다.
전달 대상은 같은 그룹에 다른 배송원 명의의 `READY` 경로가 있는 경우만 노출한다.
과거 또는 미래 그룹은 세 목록을 비워 반환하며 `acquire`, `release`, `transfer`는
`DESTINATION_BUNDLE_TRANSFER_CLOSED`로 차단한다.

반납·확보 명령 본문은 `{ "expectedVersion": "..." }`, 전달 명령 본문은
