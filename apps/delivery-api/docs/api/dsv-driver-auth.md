...
사용하지 않는다. 일반 앱 로그인 화면의 회원가입 버튼에서 로그인 ID 또는 이메일 주소,
비밀번호, 이름과 국내 휴대전화 번호를 받고 이후 로그인은 같은 식별자와 비밀번호만 사용한다.
...
`@`, 비어 있거나 점 없는 도메인을 허용하지 않는다. 비밀번호는 8~128자, 이름은 1~80자,
휴대전화 번호는 `01`로 시작하는 숫자 10~11자리다. 전화번호 구분기호와 로그인 식별자의
대문자는 서버 경계에서 정규화된다.
...
발급·검증·완료 시 해당 사업장의 활성 DSV 배송원과 연결된 활성 DriverAccount를 확인한다.
발급 후 배송원 비활성화·연결 해제·사업장 이동으로 자격을 잃으면 완료할 수 없다.
...
3회로 제한하며 발급 API 요청은 클라이언트별 15분에 10회로 제한한다. 공개 검증 API는
클라이언트별 15분에 20회, 완료 API는 10회로 제한한다.
...
로그인 ID, 이름, 전화번호와 비밀번호는 기록하지 않는다. 초기화 응답은 `Cache-Control: no-store`를
사용하고 초기화 API 요청 query 전체를 애플리케이션 로그에서 제외한다.

## 운영 활성화

인증 라우트는 `CLEVER_DSV_DRIVER_AUTH_ENABLED=true`일 때만 등록된다. 이 값이 켜진
환경에서는 `JWT_SECRET`이 누락되거나 32자보다 짧으면 서버가 기동을 거부한다.
관리자 링크 발급도 같은 인증 활성화 플래그가 켜진 경우에만 제공한다.
...
운영 Compose는 `CLEVER_TRUST_CADDY_PROXY=true`로 Caddy DNS 주소를 기동 시 확인한다.
API는 그 주소의 바로 앞 프록시 한 홉에서 온 전달 주소만 신뢰하여 클라이언트별 요청 제한을
적용한다. 직접 연결 또는 임의의 전달 헤더는 신뢰하지 않는다. Caddy 컨테이너 IP가 바뀌면
API도 재시작하여 신뢰 주소를 갱신한다.
