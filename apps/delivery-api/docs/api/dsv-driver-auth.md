...
사용하지 않는다. 가입은 로그인 ID, 비밀번호, 이름, 국내 휴대전화 번호를 받고 이후
로그인은 로그인 ID와 비밀번호만 사용한다. `residentNumberFront` API 필드는 구버전
호환을 위해 유지하지만 `null` 또는 생략할 수 있다.
...
선택적으로 전달하는 주민번호 앞자리는 숫자 7자리여야 한다. 전화번호 구분기호와 로그인
ID 대문자는 서버 경계에서 정규화된다.

## 연결 규칙

주민번호 앞 7자리 값이 있는 구버전 요청의 계정은 다음 조건이 모두 같은 활성 DSV
배송원만 확보한다.
...
`residentNumberFront`가 `null`이거나 생략되면 지문을 생성하거나 배송원을 자동으로
연결하지 않고 계정을 `UNLINKED`로 만든다. 전화번호만 입력한 신규 계정의 자동 연결은
SMS OTP 등 전화번호 소유 검증 계약이 추가된 뒤 활성화한다.

구버전 가입 시 일치 배송원이 없으면 계정은 `UNLINKED`로 만들어진다. DSV 운영자가
배송원을 나중에 추가하면 다음 성공 로그인에서 다시 같은 조건으로 연결한다. 일부 값만 같은
배송원이나 이미 다른 계정이 확보한 배송원은 연결하지 않는다.
