...
This document records how `clever-route-server` should treat location information so later implementation, evidence collection, and retention decisions are easy to review. It is an engineering control reference, not legal advice. Legal filings, terms, consent UX, and privacy notices remain a separate legal review track.
...
| `DriverProofMedia` rows and stored proof files | 365 days after occurrence/upload by default | Needed for customer support and delivery dispute handling. | Delete stored file bytes through the configured proof-media storage backend and hard-delete metadata and stop links after retention unless legal/incident hold exists; stored JPEG bytes have EXIF APP1 metadata stripped before hash/size recording and accepted media can pass through a scanner hook before persistence. |
...
- Proof-media retention cleanup removes stored bytes and hard-deletes expired metadata and stop links through the driver proof-media repository. `npm run driver:proof-media:cleanup` is available for manual or cron-style execution and persists only a sanitized aggregate `RetentionJobRun` row for private scheduler evidence; no deployed scheduler evidence exists yet.
- Proof-media scanner rejection and scan-outcome monitor hooks exist before byte/metadata persistence, and optional HTTP scanner/monitor adapters can be selected by runtime env. A scanner-free production policy must be recorded explicitly and retains authentication, route scope, image MIME/signature and size checks, and JPEG EXIF stripping; it must not be represented as malware-scanned evidence.
