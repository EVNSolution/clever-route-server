...
const dsvActiveSessionIdsMigrationName = '20260813090000_add_dsv_active_session_ids';
const dsvPasswordHistoryMigrationName = '20260813130000_add_dsv_password_history';
const dsvPasswordHistoryMigrationPath = new URL(
...
describe('G007 DSV Prisma migration history', () => {
  ...
  test('orders the additive cleanup and G009 through G011 tenant-composite repairs', async () => {
    ...
    expect(migrations).toContain(dsvActiveSessionIdsMigrationName);
    ...
    expect(migrations.indexOf(uvisTrailMaterializationMigrationName)).toBeLessThan(
      migrations.indexOf(dsvActiveSessionIdsMigrationName)
    );
    ...
    expect(migrations.indexOf('20260819223000_add_custom_route_stop_source')).toBeLessThan(
      migrations.indexOf('20260820093000_own_virtual_route_group_orders')
    );
    expect(migrations.indexOf('20260824240000_preserve_shop_redaction_receipts')).toBeLessThan(
      migrations.indexOf('20260825120000_enforce_shop_privacy_tombstone')
    );
    expect(migrations.indexOf('20260825120000_enforce_shop_privacy_tombstone')).toBeLessThan(
      migrations.indexOf('20260825130000_customer_email_operator_reconciliation')
    );
    expect(migrations.indexOf('20260825130000_customer_email_operator_reconciliation')).toBeLessThan(
      migrations.indexOf('20260825130000_driver_route_completion_invariant_rollout')
    );
    expect(migrations.indexOf('20260825130000_driver_route_completion_invariant_rollout')).toBeLessThan(
      migrations.indexOf('20260825140000_preserve_route_completion_gate_history')
    );
    expect(migrations.indexOf('20260825140000_preserve_route_completion_gate_history')).toBeLessThan(
      migrations.indexOf('20260831160000_customer_email_delivery_evidence')
    );
    ...
    expect(migrations.at(-1)).toBe('20260901070000_complete_driver_account_deletion_lifecycle');
  });

  test('keeps completion rollout gate outcomes after tenant graph deletion', async () => {
    const migration = await readFile(new URL('20260825140000_preserve_route_completion_gate_history/migration.sql', migrationsDir), 'utf8');
    expect(migration).toContain('CREATE TABLE "driver_route_completion_gate_history"');
    expect(migration).not.toContain('FOREIGN KEY');
    expect(migration).not.toContain('"shopId"');
    expect(migration).not.toContain('"routePlanId"');
    expect(migration).toContain('"retainedUntil"');
  });
  ...
  test('adds one coherent previous-password credential pair to DSV account tables', async () => {
    const migration = await readFile(dsvPasswordHistoryMigrationPath, 'utf8');

    for (const tableName of ['customer_accounts', 'dsv_admin_accounts']) {
      expect(migration).toContain(`ALTER TABLE "${tableName}"`);
      expect(migration).toContain('ADD COLUMN "previousPasswordHash" TEXT');
      expect(migration).toContain('ADD COLUMN "previousPasswordSalt" TEXT');
    }
  ...
  test('schema preserves historical DB defaults instead of planning default drops', async () => {
    ...
    expect(schema.match(/@default\(dbgenerated\("gen_random_uuid\(\)"\)\)/gu) ?? []).toHaveLength(46);
