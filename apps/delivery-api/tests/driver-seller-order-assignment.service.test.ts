...
describe('DriverSellerOrderAssignmentService', () => {
  ...
  test('acquires one indivisible SellerOrder into the authenticated driver route', async () => {
    ...
    const result = await service.acquire({ ...scope, orderId: 'order-3' });
  ...
  test('requires a vehicle on the target route instead of guessing WMS vehicle identity', async () => {
    ...
    await expect(service.acquire({ ...scope, orderId: 'order-3' }))
  ...
  test('releases assignment without deleting the SellerOrder', async () => {
    ...
    const result = await service.release({ ...scope, orderId: 'order-1' });
