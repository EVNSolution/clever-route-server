...
import { DriverSellerOrderAlreadyAcquiredError } from '../src/modules/driver/driver-seller-order-assignment.service.js';
...
describe('driver SellerOrder assignment routes', () => {
  ...
  test('returns a stable conflict code when another driver acquired the order first', async () => {
    ...
    try {
      ...
      expect(acquire).toHaveBeenCalledWith({ ...context, orderId: 'order-1' });
    } finally {
      await app.close();
    }
  });

  test('keeps release separate from deletion and delegates only the order id', async () => {
    ...
    try {
      ...
      expect(response.statusCode).toBe(200);
      expect(release).toHaveBeenCalledWith({ ...context, orderId: 'order-1' });
    } finally {
      await app.close();
    }
  });
