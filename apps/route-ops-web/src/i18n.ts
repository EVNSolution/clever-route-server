...
export const routesCopy = {
  'en-CA': {
    ...
    routeSavePublishesRoute: 'Save will make this driver-assigned draft visible to drivers.',
    noRouteChanges: 'No route changes to save.',
    returnToStoreDepotMissing: 'Return to store requires saved store coordinates.',
    stopTable: {
      actions: 'Actions',
      area: 'Area',
      customer: 'Customer',
    ...
    routeOptimization: {
      details: {
        ...
        status: 'Status',
        step: 'Step',
        timeoutBudget: 'Result wait limit',
    ...
    dragPlanOrder(orderName: string): string { return `Drag to reorder ${orderName}`; }
  },
  'ko-KR': {
    routes: '경로',
    stops: '하차지',
    ...
    saveRoute: '경로 저장',
    savingRoute: '경로 저장 중…',
    routeSavePublishesRoute: '저장하면 배정된 초안 경로가 배송원에게 표시됩니다.',
    noRouteChanges: '저장할 경로 변경사항이 없습니다.',
    returnToStoreDepotMissing: '매장 복귀는 저장된 매장 좌표가 필요합니다.',
    stopTable: {
      actions: '동작',
      area: '지역',
      customer: '고객',
    ...
    dragPlanOrder(orderName: string): string { return `${orderName} 순서 변경 드래그`; }
  }
} as const;
...
export const ordersCopy = {
  'en-CA': {
    ...
    clearFilter(label: string): string { return `Clear ${label}`; },
    ...
    completed: 'Completed',
    normal: 'Normal',
    routeDraftPanelLabel: 'New Route',
    routeDraftTitle: 'New Route',
    addPlanAction: 'Add Plan',
    orders: 'orders',
    ...
    routeValidationReasons: 'Route validation reasons',
    ...
    routeDraft: {
      ...
      dateMustMatch: 'Selected orders must share one delivery date.',
      missingRequired: 'Some selected orders are missing delivery date or type.',
      missingRequiredShort: 'Missing',
      selectedMustShareScope: 'Selected orders must share one type.',
      selectedMustShareType: 'Selected orders must share one type.'
    },
    bulkStatus: {
      matched: 'matched',
      attempted: 'attempted',
  ...
  'ko-KR': {
    ...
    filtersTitle: '주문 필터',
    clearFilters: '필터 초기화',
    clearFilter(label: string): string { return `${label} 지우기`; },
    deliveryDate: '배송 날짜',
    weekday: '요일',
    type: '타입',
    totalOrders: '총 주문 수',
    ...
    completed: '완료됨',
    normal: '정상',
    routeDraftPanelLabel: '새 경로',
    routeDraftTitle: '새 경로',
    addPlanAction: '계획 추가',
    orders: '주문',
    routeDate: '경로 날짜',
    routeName: '경로 이름',
    ...
    createRoute: '경로 생성',
    clearPlan: '계획 비우기',
    routeValidationReasons: '경로 검증 사유',
    ...
    routeDraft: {
      ...
      dateMustMatch: '선택한 주문은 하나의 배송 날짜여야 합니다.',
      missingRequired: '선택한 주문 중 배송 날짜 또는 타입이 없는 주문이 있습니다.',
      missingRequiredShort: '누락',
      selectedMustShareScope: '선택한 주문은 하나의 타입이어야 합니다.',
      selectedMustShareType: '선택한 주문은 하나의 타입이어야 합니다.'
    },
    bulkStatus: {
      matched: '일치',
      attempted: '시도',
