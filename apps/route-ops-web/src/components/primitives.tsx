...
export function MiniOrderList({ empty, orders }: { empty: string; orders: CanonicalOrderDto[] }): ReactElement {
  if (orders.length === 0) return <p className="muted">{empty}</p>;
  return <ul className="mini-list">{orders.map((order) => <li key={order.orderId}><strong>{order.orderName}</strong><small>{order.blockerReasons.join(', ') || order.deliveryDate}</small></li>)}</ul>;
}
