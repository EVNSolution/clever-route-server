...
.route-save-panel {
  height: var(--route-builder-detail-panel-height);`);
    ...
    expect(css).toContain('.route-child-sequence-card');
    expect(css).toContain('.route-child-sequence-header');
    expect(css).toContain('.route-child-sequence-row');
    expect(css).toContain('.route-child-sequence-track');
    expect(css).toContain('.route-child-sequence-footer');
    expect(css).toContain('.route-map-delete-button');
  ...
  test('Route grouping CSS keeps assignment tokens circular and the split map tall', () => {
    ...
    expect(css).toContain('grid-template-columns: repeat(');
    expect(css).toContain('var(--route-group-area-columns, 1)');
    expect(css).toContain('minmax(24px, 1fr)');
    expect(css).toContain('.route-group-area-track::before');
    expect(css).toContain('right: 84px;');
